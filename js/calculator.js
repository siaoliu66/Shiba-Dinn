function cal() {
    ...
    var finalPrice = Math.round(totalPrice + num * 240 + 50)
    // var vipPrice
//   var text = `<p>📣報價如下</br>🔷商品金額：`
//     if ($(".vip").prop("checked")) {
//         if (totalPrice >= 3000 && totalPrice < 5999) {
//             vipPrice = totalPrice - 50
//             finalPrice = Math.round(vipPrice + num * 240 + 50)
//             if (Shipping == 0) {
...
//         text += `</br>
// 🔷國際運費：1公斤240元</br>
// 🔷應付金額：<span class="totalPrice">${toCurrency(vipPrice)}</span> + <span class="Num">240*${num}</span>(重量預估${num}kg以內) + 50(超商運費) = <span class="finalPrice">${toCurrency(finalPrice)}</span></p>`
//     } else {
//         if (Shipping == 0) {
...
// 🔷國際運費：1公斤240元</br>
// 🔷應付金額：<span class="totalPrice">${toCurrency(totalPrice)}</span> + <span class="Num">240*${num}</span>(重量預估${num}kg以內) + 50(超商運費) = <span class="finalPrice">${toCurrency(finalPrice)}</span></p>`
//     }
var text = `<p>📣報價如下</br>🔷商品金額：`
if (Shipping == 0) {
        text += `<span class="Price">${toCurrency(price)}</span>`
